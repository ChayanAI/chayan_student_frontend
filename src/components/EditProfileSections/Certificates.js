...
const Certificates = ({userId, profileData, setProfileData, months}) => {
    ...
    useEffect(() => {
        (async () => {
            await axios.post(`${process.env.NEXT_PUBLIC_APP_API_IP}/proof/getpfpbyId`,{userId}).then((res)=>{
                res.data.map((item)=>{
                    setProofs((prev)=>({...prev,[item.id]: `https://storage.googleapis.com/chayan-proofs/${item.proof}/${item.id}.jpeg`}))
                })
    ...
    if (!loader) {
        ...
        return (
            <div className="rounded-lg p-10 mb-6 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-2">
                {profileData.certificates.map((x, index) => {
                    return (<div
                        ...
                        <Text name={'Certificate Name'}
                              value={profileData.certificates[index].name}
                              disp='name' list='certificates' index={index}
                              setValue={setProfileData} col={' col-span-full'} isRequired={true}/>
                        <Text name={'Date of Issuance'}
                              value={formatDate(profileData.certificates[index].issuance_date)}
                              disp='issuance_date' index={index}
                              list='certificates' isRequired={true}
                              setValue={setProfileData} type={'date'} col={3}/>
                        <Text name={"Provider's Name"}
                              value={profileData.certificates[index].provider_name}
                              disp='provider_name' list='certificates'
                              index={index} isRequired={true}
                              setValue={setProfileData} col={3}/>
                        <Text name={'Platform'}
                              value={profileData.certificates[index].platform}
                              disp='platform' list='certificates' index={index}
                              setValue={setProfileData} col={' col-span-full'}/>
                        <div className="col-span-full" onClick={()=>console.log(proofs)}>
                            ...
                            <div
                                ...
                                <div className="text-center flex items-center justify-center flex-col">
                                    {proofs[profileData.certificates[index].id] ? (
                                        <img src={proofs[profileData.certificates[index].id]} alt='profile'
                                             className='h-24 w-28 object-cover ml-2 border-2 border-black rounded-[6%]'/>) : (
                                        <PhotoIcon className="mx-auto h-12 w-12 text-gray-300" aria-hidden="true"/>)}
                                    <div className="mt-4 flex text-sm leading-6 text-gray-600">
                                        <label
                                            htmlFor={profileData.certificates[index].id}
                                            className="relative cursor-pointer rounded-md font-semibold text-indigo-600 focus-within:outline-none focus-within:ring-2 focus-within:ring-indigo-600 focus-within:ring-offset-2 hover:text-indigo-500"
                                        >
                                            <span>Upload a file</span>
                                            <input onChange={(e) => handleupload(e, profileData.certificates[index].id)}
                                                   accept="image/jpeg, image/png"
                                                   id={profileData.certificates[index].id} name="file-upload"
